...
class MyGUI:
    ...
    def newprediction(self):
        ...
        if len(self.played)<2:
            ...
            pairs = list(zip(self.played,self.answered))
            simpletriples = list(zip(self.played,self.played[1:],self.played[2:]))
            #print(simpletriples)
            triplefreqs = [ [[0 for k in range(3)]for j in range(3)] for i in range(3)]
            for (a,b,c) in simpletriples:
                triplefreqs[a][b][c]+=1

            letzter = self.played[-1]
            vorletzter = self.played[-2]
            situation = triplefreqs[vorletzter][letzter]
            ...
            self.prediction = rd.choice(gute_züge)
